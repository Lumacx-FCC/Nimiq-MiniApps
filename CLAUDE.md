...
- Pages = modules: `Landing` (mockup-faithful; logged-in users get a character gallery with Scene/Video/Talk actions), `CharacterStudio` (cascade flow: upload → analyze → sheet → generate → save reveals sections progressively), `Scenes`, `Videos`, `RoleplayStudio` (Aeternum port), `Login`, `Credits`, `Profile` (`/profile` — balance, connected logins, link/unlink UI; needs both `authProviders.ts` and `accountLink.ts`).
...
**Shipped**: server-side credit granting with on-chain tx verification, signed-challenge wallet login, production prices/treasuries, real Firebase Auth for email/Google, account linking (wallet ↔ email/Google, pairing-code based), cloud sync for character sheets + custom avatars.

**Still open**: cloud sync for scenes/videos (Stages 2/3, deliberately deferred), rate limiting beyond the account-linking endpoints, real Node host for `server/api.ts` (handlers only use fetch/env — portable), Firestore TTL policies for `rate_limits`/`link_codes`/`link_tickets` (console-side config, not code).
