...
### Payment rails beyond NIM/USDT (`functions/src/paypal/`, `functions/src/onvo/`)

Two additional rails exist for users without a crypto wallet. Both are hidden for Nimiq wallet sign-ins (`user?.provider !== 'nimiq'` gates in `Credits.tsx` — the mini-app competition rules prohibit third-party payment providers inside the wallet WebView, so visibility is gated per signed-in identity rather than a build-time flag) and both credit the user **only** via a server-side webhook, never the client's own say-so:

- **PayPal Hosted Buttons** (`showPaypal`, `mode === 'card'`) — 3 fixed-price buttons configured directly in the PayPal Business dashboard, embedded via `core-modules/.../credits/payPaypal.ts`'s `renderHostedButton`. `functions/src/paypal/webhook.ts` verifies the signature (a call to PayPal's own `/v1/notifications/verify-webhook-signature`, not a local HMAC), then on `PAYMENT.CAPTURE.COMPLETED` matches capture → order id → payer email → Firebase Auth user → that uid's pending PayPal order by amount (Hosted Buttons can't carry our own order reference at render time, hence the email-matching workaround). **The webhook must be registered on the auto-provisioned "NVP SOAP Webhooks" app** (API Credentials → NVP/SOAP API apps in the PayPal Developer Dashboard) — Hosted Buttons transactions don't report to the REST app under "Apps & Credentials" at all, confirmed by a full day of zero webhook events there. Full reusable pattern + this gotcha: user-level skill `paypal-hosted-buttons`.
- **ONVO Pay / SINPE Móvil** (`showOnvo`, `mode === 'sinpe'`, Costa Rica only) — `functions/src/onvo/{client,routes,webhook}.ts`. Unlike PayPal, ONVO lets a payment intent carry our own order id via `metadata`, so its webhook is a direct lookup, not a fuzzy match. `POST /api/onvo/confirm` creates a `mobile_number` payment method using the **customer's own** phone + national ID (`identification`/`identificationType` — required fields ONVO's rendered docs omit entirely; a missing one 400s, enum documented in `onvo/client.ts`) and a CRC payment intent (USD pack price converted at order-creation time via `getUsdCrcRate()` in `functions/src/config.ts`, frozen into the order like NIM's rate), then confirms it — that only starts the SINPE Móvil request toward the customer's bank, it does not mean the money arrived; only the webhook (`payment-intent.succeeded`) grants, with the reconciler's `verifyOnvo` polling as a fallback for a missed webhook. Client-side, `runOnvoPurchase` (`src/core/credits.ts`) reuses NIM/USDT's exact `watchOrder`/confirming-payment UI machinery rather than a separate flow, since ONVO also resolves asynchronously. Full reusable pattern + why ONVO over TiloPay: user-level skill `onvo-pay-sinpe-movil`.
...
- **Credits economy** (constants in `src/core/config.ts`): sheet renders 5 free anonymous, then login → 5 welcome credits (granted once per new ledger in `loadCreditsFor`) → 1 credit/render; scenes 5 free then 5 credits; sprites 3; talking 1/min; videos 100. Spends go through the single shared `credits.spend()`; purchases are real wallet transactions at live early-bird production prices. On the Credits page NIM is the primary rail (never disabled waiting on the CoinGecko quote — `buyWithNim` fetches its own rate), USDT second with a Polygon-network warning, PayPal and ONVO Pay (SINPE Móvil, Costa Rica) third/fourth for non-wallet sign-ins — see "Payment rails beyond NIM/USDT" above. Prices/treasuries/NIM bonus have one source of truth, `functions/src/sharedPricing.ts` (plain literal exports, no imports of its own) — both `src/core/config.ts` (via a relative import) and `functions/src/config.ts` (re-export) read from it; edit that one file, not two, when a pack or treasury changes. PayPal/ONVO price off `REGULAR_PACKS` in that same file (no early-bird discount on those rails).
- **Responsive layout**: `.page-shell`'s max-width was a hard 480px everywhere (`global.css`) regardless of actual viewport — it now scales up past phone-portrait widths (640px at ≥640px, 760px at ≥900px) via media queries instead of staying pinned to a phone-card width on tablet/desktop; `.wide` pages keep their own larger cap. `AppHeader.tsx`'s wordmark visibility is **not** a fixed Tailwind breakpoint — that was tried three times (400px → 480px → 560px) and kept getting outrun by variable content (the credit balance growing past one digit, or a longer per-page `title`). It now measures the actual rendered widths (logo + full untruncated wordmark + icon row) via `ResizeObserver` and shows the wordmark in full or not at all, never truncated mid-word ("Other ..."). Don't reintroduce a magic-pixel breakpoint here — it will eventually break again the same way.
...
**Shipped**: server-side credit granting with on-chain tx verification, signed-challenge wallet login, production prices, real Firebase Auth for email/Google, account linking (wallet ↔ email/Google, pairing-code based), cloud sync for character sheets + custom avatars + scenes/videos, an admin-gated credit-grant endpoint + `/promos_management` page for contest/promo payouts (deployed and confirmed live-tested 19 Aug 2026). Production treasury addresses (rotated to real wallets in `functions/src/sharedPricing.ts`); rate limiting on `/api/auth/challenge`, `/api/orders`, the AI generation routes (uid-or-IP keyed via `app.set("trust proxy", true)`), new-account creation on `/api/account/resolve`, and `/api/credits/record-purchase`, alongside the pre-existing account-linking endpoints; welcome credits gated on email verification for the `email` provider (closes the free-account farming path); `recordPurchase` hardened to recompute credits server-side from a matching pack instead of trusting the client's number; the order-claim client call now retries with backoff and persists an unclaimed claim to `localStorage` for retry on session restore instead of silently losing it after a real on-chain payment (PR #28). PayPal Hosted Buttons checkout + webhook crediting (20-21 Aug 2026) and ONVO Pay SINPE Móvil checkout + webhook crediting for Costa Rica (2 Sep 2026) — both confirmed live with real payments; see "Payment rails beyond NIM/USDT" above.

**Still open**: real Node host for `server/api.ts` (handlers only use fetch/env — portable), Firestore TTL policies for `rate_limits`/`link_codes`/`link_tickets` (console-side config, not code). A few internal endpoints remain without rate limiting (`/api/credits/spend`, `/api/credits/migrate`, `/api/admin/grant-credits`, `/api/onvo/confirm`) — lower risk since they already require an authenticated (or admin) session.
